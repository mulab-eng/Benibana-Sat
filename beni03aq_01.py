...
def quaternion_to_rotation_matrix(qu):
    x, y, z, w = qu
...
def rotate_vector_by_quaternion(qu, vec):
    """
    ...
    """
    # ベクトルを純粋なクォータニオンに変換
    v_quat = np.array([vec[0], vec[1], vec[2], 0.0], dtype=float)
    
    # クォータニオンの共役（正規化されたクォータニオンの逆元）
    qu_conj = np.array([-qu[0], -qu[1], -qu[2], qu[3]], dtype=float)
    
    # q * v * q^(-1) を計算
    temp = multiply_quaternions(qu, v_quat)
    result = multiply_quaternions(temp, qu_conj)
...
class Scene:
    def __init__(self, render):
        ...
        # 面付きの立方体モデルを読み込む
        solid_cube = loader.loadModel("models/misc/rgbCube")
        solid_cube.setScale(0.2)
        solid_cube.clearTexture()  # テクスチャを無効化（色を反映させるため）
        solid_cube.setTransparency(True)  # ★透明化を有効化（必須）
        solid_cube.setColor(0.85, 0.85, 0.85, 0.95)
        solid_cube.reparentTo(self.cube)  
...
class CubeApp(ShowBase):
    def __init__(self):
        ...
        self.qu = np.array([0.0, 0.0, 0.0, 1.0], dtype=float)  # [x, y, z, w]
    ...
    def _simulate_step(self, dt):
        ...
        R = quaternion_to_rotation_matrix(self.qu)

        # 地磁気を固定し，磁気トルカと地磁気の外積をトルクとする．
        self.magnetic = R.T @ np.array([-0.1, 0.1, 0.0]) # 地磁気ベクトルを機体座標系に変換
        torque_x = np.cross(self.toruca_x, self.magnetic) # 磁気トルクの計算 tau = m x B
        torque_y = np.cross(self.toruca_y, self.magnetic) # 磁気トルクの計算 tau = m x B
        ...
        omega_qu = np.array([self.omega[0], self.omega[1], self.omega[2], 0.0], dtype=float) 
             #角速度ベクトルをクォータニオンに変換
        qdot = 0.5 * multiply_quaternions(self.qu, omega_qu) #qdot = 0.5 * q * omega_qu
        self.qu += qdot * dt
        q_norm = np.linalg.norm(self.qu)
        if q_norm > 1e-12: # クォータニオンの正規化
            self.qu /= q_norm
        else:
            self.qu = np.array([0.0, 0.0, 0.0, 1.0], dtype=float)

        R = quaternion_to_rotation_matrix(self.qu) #クォータニオンから回転行列に変換
        # axis = [1, 0, 0]
        # theta = np.pi / 6
        ...
        # トルクベクトル描画（クォータニオンを使用）
        disp_torque = 5.0 * rotate_vector_by_quaternion(self.qu, self.torque) #描画用のトルクデータ
        ...
        # --- クォータニオンを Panda3D の Quat に変換して姿勢をセット ---
        qx, qy, qz, qw = self.qu  # numpy → Panda3D の順番は同じ (x,y,z,w)
        quat_p3d = Quat(qw, qx, qy, qz)  # Panda3D は (w,x,y,z) の順でコンストラクタ
        self.scene.cube.setQuat(quat_p3d)
