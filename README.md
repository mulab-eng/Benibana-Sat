## 衛星の回転運動と姿勢制御

人工衛星を一つの剛体ととらえ，力学の法則を適用して回転運動を考えることができます．
Pythonを用いて衛星の回転運動をシミュレーションしていますので，その概要を説明します．




### <a href="Python.md" target="_new" rel="noopener noreferrer">Python</a>

### <a href="座標系.md" target="_blank" rel="noopener noreferrer">座標系</a>

### <a href="外積について.md" target="_blank" rel="noopener noreferrer">外積について</a>


### <a href="運動方程式.md" target="_blank" rel="noopener noreferrer">運動方程式</a>

### <a href="回転行列と姿勢表現.md" target="_blank" rel="noopener noreferrer">回転行列と姿勢表現</a>


### <a href="磁気トルカによるトルク.md" target="_blank" rel="noopener noreferrer">磁気トルカによるトルクの特徴</a>


### <a href="回転運動のシミュレーション_架空.md" target="_blank" rel="noopener noreferrer">回転運動のシミュレーション（架空のデータ）</a>

### <a href="回転運動のシミュレーション_実機.md" target="_blank" rel="noopener noreferrer">回転運動のシミュレーション（実機に近いデータ）</a>

### <a href="磁気の勉強.md" target="_blank" rel="noopener noreferrer">磁気の勉強</a>



